...
class MouseMoverTableBased(MouseMover):
    ...
    def mouse_action(self, decision, topleftcorner,bet_val = 0):
        ...
        if bet_val == 1:
            if decision == 'Bet Bluff' or decision == 'Bet pot' or decision == 'Bet half pot':
                decision = 'Bet div 3'
        elif bet_val ==2:
            ...
            if decision == 'Bet Bluff' or decision == 'Bet pot' or decision == 'Bet half pot' or decision == 'Bet div 3':
                decision = 'Bet'
            # decision = 'Call'
        # else:
            # decision = 'Fold'
        ...
        if decision == "All In":
            ...
            for action in self.coo[decision]:
                if i == 0:
                    ...
                    time.sleep(np.random.uniform(0, action[1], 1)[0])
                    xsrand = int(np.random.uniform(0, action[4], 1)[0])
                    ysrand = int(np.random.uniform(0, action[5], 1)[0])
                    self.mouse.press(action[2]+ tlx + xsrand, action[3]+ tly + ysrand)
                    i += 1
                    # self.mouse.move(xd + xdrand, yd + ydrand)
                elif i == 2:
                    time.sleep(np.random.uniform(0, action[1], 1)[0])
                    xsrand = int(np.random.uniform(0, action[4], 1)[0])
                    ysrand = int(np.random.uniform(0, action[5], 1)[0])
                    self.mouse.release(action[2]+ tlx + xsrand, action[3]+ tly + ysrand)
                    i += 1
                elif i == 3:
                    time.sleep(np.random.uniform(0, action[1], 1)[0])
                    xsrand = int(np.random.uniform(0, action[4], 1)[0])
                    ysrand = int(np.random.uniform(0, action[5], 1)[0])
                    self.mouse.click(action[2]+ tlx + xsrand, action[3]+ tly + ysrand)
                    i += 1
        else:
            for action in self.coo[decision]:
