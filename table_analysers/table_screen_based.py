...
class TableScreenBased(Table):
    ...
    def get_other_player_funds(self, p):
        ...
        for i, fd in enumerate(func_dict, start=0):
            ...
            for j in suits :
                ...
                img = cv2.cvtColor(np.array(pil_image), cv2.COLOR_BGR2RGB)
                count, points, bestfit, _ = self.find_template_on_screen(self.callnumber, img, 0.06)
                if count != 0:
                    for point in points:    
                        points_x.update({
                            point[0]:j
    ...
    def get_empty_status(self):
        ...
        for i, fd in enumerate(func_dict, start=0):
            ...
            pil_image = self.crop_image(self.entireScreenPIL, fd[0], fd[1],
                                        fd[2], fd[3])
            img = cv2.cvtColor(np.array(pil_image), cv2.COLOR_BGR2RGB)
            count, points, bestfit, minvalue = self.find_template_on_screen(self.empty_player, img, 0.04)
    ...
    def get_lost_everything(self, h, t, p, gui_signals, mouse):
        if self.tbl == 'SN':
