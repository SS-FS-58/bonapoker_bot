...
class Table(object):
    ...
    def load_templates(self, p):
        ...
        for x in values:
            for y in suites:
                name = "pics/" + self.tbl[0:2] + "/cards/" + x + y + ".png"
        ...
        name = "pics/" + self.tbl[0:2] + "/empty_status.png"
        template = Image.open(name)
        self.empty_player = cv2.cvtColor(np.array(template), cv2.COLOR_BGR2RGB)
    ...
    def take_screenshot(self, initial, p):
        ...
        if (self.whnd == 0):
            # In case of static
            filename = r"f:\24_Poker_bot\04_backup\Test\test5.png"
        ...
        # if (self.whnd == 0):
            try:
                ...
                # self.path = r'f:\24_Poker_bot\02_source\Poker_bot\temp_screenshot\test{}.png'.format(datetime.now().strftime("%d%m%Y%H%M%S"))
